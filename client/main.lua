...
Citizen.CreateThread(function()
    AddTextEntry("Tattoshop", "Tattoo Shop")
    for k, v in pairs(Config.Shops) do
        ...
        BeginTextCommandSetBlipName("Tatto Shop")
...
function IsMenuOpen()
    return (BnzMenu.IsMenuOpened('tattoo') or string.find(tostring(BnzMenu.CurrentMenu() or ""), "ZONE_"))
end
...
Citizen.CreateThread(function()
    BnzMenu.CreateMenu("tattoo", "Tattoo Shop", function()
        return CloseTattooShop()
    end)
    BnzMenu.SetSubTitle('tattoo', "Categories")
    
    for k, v in ipairs(Config.TattooZone) do
        BnzMenu.CreateSubMenu(v[1], "tattoo", v[2])
        BnzMenu.SetSubTitle(v[1], v[2])
    end
    ...
    while true do
        ...
        if BnzMenu.IsMenuOpened('tattoo') then
            CanSleep = false
            for k, v in ipairs(Config.TattooZone) do
                BnzMenu.MenuButton(v[2], v[1])
            end
            ClearPedDecorations(PlayerPedId())
            for k, v in pairs(currentTattoos) do
                if v.Count ~= nil then
        ...
        for k, v in ipairs(Config.TattooZone) do
            if BnzMenu.IsMenuOpened(v[1]) then
                CanSleep = false
                ...
                for _, tattoo in pairs(Config.ListTattos) do
                    if tattoo.Zone == v[1] then
                        if GetEntityModel(PlayerPedId()) == 'mp_m_freemode_01' then
                            if tattoo.HashNameMale ~= '' then
                                ...
                                if found then
                                    local clicked, hovered = BnzMenu.SpriteButton(GetLabelText(tattoo.Name), "commonmenu", "shop_tattoos_icon_a", "shop_tattoos_icon_b")
                                    if clicked then
                                        RemoveTattoo(tattoo.HashNameMale, tattoo.Name)
                                    end
                                else
                                    local price = math.ceil(tattoo.Price / 20) == 0 and 100 or math.ceil(tattoo.Price / 20)
                                    local clicked, hovered = BnzMenu.Button(GetLabelText(tattoo.Name), "~HUD_COLOUR_GREENDARK~$" .. price)
                                    if clicked then
