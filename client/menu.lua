...
local function debugPrint(text)
    if BnzMenu.debug then
        Citizen.Trace('[BnzMenu] '..tostring(text))
    end
end
...
-- API
function BnzMenu.CreateMenu(id, title, closeCallback)
    -- Default settings
    menus[id] = { }
...
function BnzMenu.CreateSubMenu(id, parent, subTitle, closeCallback)
    if menus[parent] then
        BnzMenu.CreateMenu(id, menus[parent].title)
...
function BnzMenu.CurrentMenu()
    return currentMenu
end

function BnzMenu.MenuTable()
    return menus
end


function BnzMenu.OpenMenu(id)
    if id and menus[id] then
...
function BnzMenu.IsMenuOpened(id)
    return isMenuVisible(id)
end


function BnzMenu.IsAnyMenuOpened()
...
function BnzMenu.IsAnyMenuWithTitleOpened(subtitle)
...
function BnzMenu.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function BnzMenu.IsThisMenuAboutToBeClosed(id)
    if menus[id] then
...
function BnzMenu.Button(text, subText)
...
function BnzMenu.DisabledButton(text, subText)
    local buttonText = text
    if subText then
        buttonText = '{ '..tostring(buttonText)..', '..tostring(subText)..' }'
    end
...
function BnzMenu.SpriteButton(text, textDict, sprite, focusSprite)
    local buttonText = text
...
function BnzMenu.SpriteMenuButton(text, textDict, sprite, focusSprite, id)
    if menus[id] then
        local clicked, hovered = BnzMenu.SpriteButton(text, textDict, sprite, focusSprite)
        if clicked then
            setMenuVisible(currentMenu, false)
...
function BnzMenu.ComboBox(text, items, currentIndex, selectedIndex, callback, displaycb)
    ...
    if BnzMenu.Button(text, selectedItem) then
...
function BnzMenu.MenuButton(text, id, secondtext)
    if menus[id] then
		local clicked, hovered = BnzMenu.Button(text, (secondtext and secondtext or "→")) 
		if clicked then
            setMenuVisible(currentMenu, false)
            setMenuVisible(id, true, true)
...
function BnzMenu.SwitchMenu(id)
	setMenuVisible(currentMenu, false)
    setMenuVisible(id, true, true)
end

function BnzMenu.SetDescription(text)
	currentDesc = tostring(text)
end

function BnzMenu.CheckBox(text, bool, callback) 

    local sprite = bool and "shop_box_tick" or "shop_box_blank"
    local focusSprite = bool and "shop_box_tickb" or "shop_box_blankb"

    if BnzMenu.SpriteButton(text, "commonmenu", sprite, focusSprite) then
        bool = not bool
...
function BnzMenu.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end


function BnzMenu.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end


function BnzMenu.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end


function BnzMenu.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end


function BnzMenu.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', { r, g, b, a or  menus[id].titleColor[4] })
end


function BnzMenu.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', { r, g, b, a or menus[id].titleBackgroundColor[4] })
end


function BnzMenu.UseSpriteAsBackground(id, textDict, sprite, r, g, b, a, stillDrawText)
